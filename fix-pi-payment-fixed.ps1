...
foreach ($secret in $secrets.GetEnumerator()) {
    try {
        ...
        $secretCommand = "supabase secrets set `"$($secret.Key)=$($secret.Value)`""
        Invoke-Expression $secretCommand | Out-Null
...
$baseUrlTemplate = "https://{0}.supabase.co/functions/v1"
$baseUrl = $baseUrlTemplate -f $PROJECT_REF
